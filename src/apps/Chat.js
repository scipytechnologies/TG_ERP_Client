...
export default function Chat() {
  ...
  useEffect(() => {
    setSocket(io("http://localhost:5009"));
    // dispatch(setSocket(io("http://localhost:5009")))
  }, []);

  useEffect(() => {
    console.log("connection Requested");
    setSenderid(user.id);
    socket?.emit("chatApp", user.id);
  }, [socket, user]);
  ...
  const [receiverid, setReceiverid] = useState("64786e44f4d8c39707de18c6");
  ...
  const SendMessage = (form) => {
    ...
    console.log(senderid, messageInput, receiverid);
    if (messageInput.trim() !== "" && receiverid.trim() !== "") {
