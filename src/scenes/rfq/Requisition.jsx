...
function PurchaseRequisition() {
  ...
  async function purchaseRequisitiondetails() {
    const res = await mainservice.purchaseRequisitiondetails(index.PurchaseRequisitionID);
    // console.log('RFQ Purchase Order Details ' + JSON.stringify(res))
    console.log(res.data.purchaseRequisition)
  ...
  async function deletepurchaseRequisition(id) {
    const res = await mainservice.deletepurchaseRequisition(index.PurchaseRequisitionID, id);
    if (res.data != null) {
        console.log("deleted");
        purchaseRequisitiondetails()
    }
...
const onDeleteHandler = (item) => {
    ...
    deletepurchaseRequisition(item._id);
}
        ...
        <div className="d-md-flex align-items-center justify-content-between mb-4">
          <div>
            <ol className="breadcrumb fs-sm mb-1">
              ...
              <li className="breadcrumb-item active" aria-current="page">Purchase Requisition</li>
            </ol>
            <h4 className="main-title mb-0">Purchase Requisition</h4>
          </div>
        </div>
          ...
          <Card.Body>
            <Grid
              data={data !== undefined ? data.map((item) => [
                item.Type,
                item.ItemCategory,
                item.ItemCatalogAndDescription,
                      ...
                      <Button className='p-0' variant="white">
                        ...
                        <Dropdown drop="end">
                          ...
                          <Dropdown.Menu>
                            ...
                            <Dropdown.Item onClick={()=>navigate(`/dashboard/rfq/purchaseRequisition/?id=${item._id}`)}>Edit</Dropdown.Item>
              ...
              columns={['Type', 'Item Category', 'Item Catalog And Description', 'Action']}
              search={true}
              pagination={true}
              sort={true}
              resizable={true}
